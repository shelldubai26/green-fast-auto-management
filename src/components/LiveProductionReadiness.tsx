...
export default function LiveProductionReadiness({lang}:{lang:Lang}){
 ...
 return <main className="lpr-page">
  <section className="lpr-hero"><div><span><ShieldCheck size={15}/> LIVE SALES AI • V0.7</span><h1>{zh?'生产上线准备中心':'Centre de préparation production'}</h1><p>{zh?'这里只检查系统上线条件。个人 TikTok 账号、视频数据和同步操作统一放到「直播销售 AI」。':'Cette page vérifie uniquement les conditions techniques. Le compte TikTok personnel, les vidéos et la synchronisation sont regroupés dans LIVE SALES AI.'}</p></div><button onClick={()=>void load()}><RefreshCw size={16}/>{zh?'重新检查':'Revérifier'}</button></section>
