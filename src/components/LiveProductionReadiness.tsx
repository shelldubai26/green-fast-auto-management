...
import TikTokRoutingModeCard from './TikTokRoutingModeCard'
...
export default function LiveProductionReadiness({lang}:{lang:Lang}){
 ...
 return <main className="lpr-page">
  <section className="lpr-hero"><div><span><ShieldCheck size={15}/> LIVE SALES AI • V0.7</span><h1>{zh?'生产上线准备中心':'Centre de préparation production'}</h1><p>{zh?'这里只检查系统是否具备上线条件：数据源、TikTok授权、成本数据、真实样本和QA。经营表现已移到直播作战总控。':'Cette page vérifie uniquement les conditions techniques de mise en production : source de données, OAuth TikTok, coûts, échantillons réels et QA. Les performances business sont désormais dans le Command Center.'}</p></div><button onClick={()=>void load()}><RefreshCw size={16}/>{zh?'重新检查':'Revérifier'}</button></section>
  ...
  {data&&<>
   ...
   <TikTokRoutingModeCard lang={lang}/>
