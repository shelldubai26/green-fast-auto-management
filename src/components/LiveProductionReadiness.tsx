...
import TikTokContentDashboard from './TikTokContentDashboard'
...
export default function LiveProductionReadiness({lang}:{lang:Lang}){
 ...
 return <main className="lpr-page">
  <section className="lpr-hero"><div><span><ShieldCheck size={15}/> LIVE SALES AI • V0.7</span><h1>{zh?'生产上线准备中心':'Centre de préparation production'}</h1><p>{zh?'把“能演示”与“能正式上线”分开检查：数据源、TikTok授权、车辆成本、真实成交样本和QA。':'Sépare clairement la démo de la production : source LIVE, OAuth TikTok, coûts véhicule, résultats réels et QA.'}</p></div><button onClick={()=>void load()}><RefreshCw size={16}/>{zh?'重新检查':'Revérifier'}</button></section>
  ...
  {data&&<>
   ...
   <TikTokContentDashboard lang={lang}/>
