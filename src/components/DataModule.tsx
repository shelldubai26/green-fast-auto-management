...
const photoUrls=(value:unknown)=>String(value||'').split(/[\n,]+/).map(x=>x.trim()).filter(x=>/^https?:\/\//i.test(x))
...
export default function DataModule({id,lang,role,userId}:{id:string;lang:Lang;role:Role;userId?:string}){
 ...
 {editing!==undefined&&(id==='content'?<ContentEditor lang={lang} row={editing} userId={userId} close={()=>setEditing(undefined)} saved={()=>{setEditing(undefined);void load()}}/>:<Editor lang={lang} fields={editableFields} configId={id} row={editing} userId={userId} close={()=>setEditing(undefined)} saved={()=>{setEditing(undefined);void load()}}/>)}{viewing&&<Detail lang={lang} fields={fields} row={viewing} close={()=>setViewing(null)}/>}</main>
}
...
function Editor({lang,fields,configId,row,userId,close,saved}:{lang:Lang;fields:Field[];configId:string;row:Row|null;userId?:string;close:()=>void;saved:()=>void}){
 ...
 const submit=async(e:React.FormEvent)=>{
  ...
  try{
   ...
   const nextForm={...form}
   if(uploaded.length){const existing=String(nextForm.photos||'').trim();nextForm.photos=[existing,...uploaded].filter(Boolean).join('\n')}
   const clean=Object.fromEntries(Object.entries(nextForm).map(([k,v])=>[k,v===''?null:v]))
   if(configId==='pricing'&&!row)clean.requested_by=userId;if(configId==='orders'&&!row)clean.salesperson_id=clean.salesperson_id||userId;if(configId==='crm'&&!row)clean.assigned_to=clean.assigned_to||userId
   ...
   saved()
  }catch(err:any){setError(err?.message||err?.details||err?.hint||JSON.stringify(err))}finally{setSaving(false)}
 }
...
function Detail({lang,fields,row,close}:{lang:Lang;fields:Field[];row:Row;close:()=>void}){
 const t=ui[lang],photos=photoUrls(row.photos),[selected,setSelected]=useState(0)
 const visibleFields=fields.filter(f=>f.key!=='photos')
 return <div className="modal-layer"><div className="modal detail"><div className="modal-head"><div><small>{t.details}</small><h2>{display(row[fields[0]?.key],lang)}</h2></div><button onClick={close}><X/></button></div>
 <div className="detail-grid">{photos.length>0?<div className="vehicle-gallery"><small>{lang==='fr'?'Photos du véhicule':'车辆图片'}</small><div className="vehicle-gallery-main"><img src={photos[Math.min(selected,photos.length-1)]} alt={String(row.model||row.make||'Vehicle')}/><span className="vehicle-gallery-count">{selected+1} / {photos.length}</span></div>{photos.length>1&&<div className="vehicle-gallery-thumbs">{photos.map((url,i)=><button type="button" className={selected===i?'active':''} key={url} onClick={()=>setSelected(i)}><img src={url} alt={`Photo ${i+1}`}/></button>)}</div>}</div>:fields.some(f=>f.key==='photos')&&<div className="vehicle-gallery-empty">{lang==='fr'?'Aucune photo enregistrée pour ce véhicule.':'这辆车暂时没有上传图片。'}</div>}
 {visibleFields.map(f=><div key={f.key}><small>{f[lang]}</small><strong className={f.key==='documents'?'photo-links':''}>{display(row[f.key],lang)}</strong></div>)}</div><div className="form-actions"><button className="secondary" onClick={close}><ArrowLeft size={15}/>{t.back}</button></div></div></div>
}
