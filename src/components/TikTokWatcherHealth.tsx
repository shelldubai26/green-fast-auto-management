...
export default function TikTokWatcherHealth({lang,role}:{lang:Lang;role:Role}){
  ...
  const stats=useMemo(()=>({
    ...
    ok:rows.filter(x=>x.last_success_at&&!x.last_error).length,
  ...
  return <div className="watcher-health">
    <div><Activity size={16}/><span>{txt('Watcher Engine','抓取引擎')}</span><b>{rows.length?txt('Actif · V0.5','运行中 · V0.5'):txt('En attente','待启动')}</b></div>
