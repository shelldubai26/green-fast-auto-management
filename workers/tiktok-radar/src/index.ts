...
function normalizeLiveProviderError(message:string){
  if(!EULER_SIGN_API_KEY && /Failed to retrieve live status from all sources/i.test(message)){
    return {error:'euler_api_key_required_or_tiktok_status_unavailable',outcome:'provider_auth_required'}
  }
  if(/401|403|unauthor|api.?key|quota|rate.?limit/i.test(message)){
    return {error:message,outcome:'provider_auth_or_quota_error'}
  }
...
async function scanLive(a:WatchAccount){
  ...
  try{
    ...
    await finishRun(runId,{items_seen:items,candidates_found:candidates,inserted_count:inserted,error:null,outcome,live_status:live,metadata:{username:a.username,tier:a.watch_tier,scan_interval_minutes:a.scan_interval_minutes||tierMinutes[a.watch_tier],euler_key_configured:Boolean(EULER_SIGN_API_KEY)}})
    console.log(JSON.stringify({event:'live_scan',username:a.username,live,items,candidates,new_leads:inserted,outcome,eulerKey:Boolean(EULER_SIGN_API_KEY)}))
  }catch(e){
    ...
    await finishRun(runId,{error:normalized.error,outcome:normalized.outcome,metadata:{username:a.username,euler_key_configured:Boolean(EULER_SIGN_API_KEY),raw_error:raw}})
